...
class CocoCSPVisDataset(CustomDataset):
    ...
    def _parse_ann_info(self, ann_info, with_mask=True):
        ...
        for i, ann in enumerate(ann_info):
            ...
            x1, y1, w, h = ann['vis_bbox']
            #if ann['area'] <= 0 or w < 1 or h < 1:
            #    continue
            if w < 1 or h < 1:
                continue
            vis_bbox = [x1, y1, x1 + w - 1, y1 + h - 1]

            if ('iscrowd' in ann and ann['iscrowd']) or ('ignore' in ann and ann['ignore'] == 1):
                gt_bboxes_ignore.append(bbox)
            else:
                gt_bboxes.append(bbox)
                gt_vis_bboxes.append(vis_bbox)
        ...
        if gt_bboxes:
            ...
            gt_vis_bboxes = np.array(gt_vis_bboxes, dtype=np.float32)
            gt_labels = np.array(gt_labels, dtype=np.int64)
        else:
            gt_bboxes = np.zeros((0, 4), dtype=np.float32)
            gt_vis_bboxes = np.zeros((0, 4), dtype=np.float32)
        ...
        ann = dict(
            bboxes=gt_bboxes, vis_bboxes=gt_vis_bboxes, labels=gt_labels, bboxes_ignore=gt_bboxes_ignore)
    ...
    def prepare_train_img(self, idx):
        ...
        if self.mixup and self.mixup_prob > np.random.rand():
            ...
            if target_idx != idx:
                ...
                gt_vis_bboxes = np.concatenate((gt_vis_bboxes, ann_t["vis_bboxes"]))
        ...
        oimg, ogt_bboxes, ogt_vis_bboxes, ogt_labels, ogt_bboxes_ignore = augment(img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore, self.img_scales[0], small_box_to_ignore=self.small_box_to_ignore)
        while oimg.shape[0] != self.img_scales[0][1] or oimg.shape[1] != self.img_scales[0][0]:
            oimg, ogt_bboxes, ogt_vis_bboxes, ogt_labels, ogt_bboxes_ignore = augment(img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore, self.img_scales[0], small_box_to_ignore=self.small_box_to_ignore)
            print("Target size  not met, trying again!!!")
        img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore =  oimg, ogt_bboxes, ogt_vis_bboxes, ogt_labels, ogt_bboxes_ignore
        ...
        # pos_map, scale_maps and offset_maps based on full boxes
        ...
        data = dict(
            ...
            gt_vis_bboxes=DC(to_tensor(gt_vis_bboxes)))
        if self.proposals is not None:
...
def resize_image(image, gt_bboxes, gt_vis_bboxes, igs, scale=(0.4, 1.5)):
    ...
    if len(gt_bboxes) > 0:
        ...
        gt_bboxes[:, 0:4:2] *= ratio
        gt_bboxes[:, 1:4:2] *= ratio
        gt_vis_bboxes = np.asarray(gt_vis_bboxes, dtype=float)
        gt_vis_bboxes[:, 0:4:2] *= ratio
        gt_vis_bboxes[:, 1:4:2] *= ratio
    ...
    return image, gt_bboxes, gt_vis_bboxes, igs


def random_crop(image, gt_bboxes, gt_vis_bboxes, gt_labels, igs, crop_size, limit=8, small_box_to_ignore=False):
    img_height, img_width = image.shape[0:2]
    crop_h, crop_w = crop_size

    if len(gt_bboxes) > 0:
        sel_id = np.random.randint(0, len(gt_bboxes))
        sel_center_x = int((gt_bboxes[sel_id, 0] + gt_bboxes[sel_id, 2]) / 2.0)
        sel_center_y = int((gt_bboxes[sel_id, 1] + gt_bboxes[sel_id, 3]) / 2.0)
    else:
    ...
    if len(gt_bboxes) > 0:
        ...
        gt_bboxes[:, 1:4:2] = np.clip(gt_bboxes[:, 1:4:2], 0, crop_h)
        gt_vis_bboxes[:, 0:4:2] -= crop_x1
        gt_vis_bboxes[:, 1:4:2] -= crop_y1
        gt_vis_bboxes[:, 0:4:2] = np.clip(gt_vis_bboxes[:, 0:4:2], 0, crop_w)
        gt_vis_bboxes[:, 1:4:2] = np.clip(gt_vis_bboxes[:, 1:4:2], 0, crop_h)
        ...
        gt_vis_bboxes = gt_vis_bboxes[keep_inds]
    ...
    return cropped_image, gt_bboxes, gt_vis_bboxes, gt_labels, igs


def random_pave(image, gt_bboxes, gt_vis_bboxes, gt_labels, igs, pave_size, limit=8, small_box_to_ignore=False):
    img_height, img_width = image.shape[0:2]
    ...
    return paved_image, gt_bboxes, gt_vis_bboxes, gt_labels, igs


def augment(img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore, size_train, small_box_to_ignore=False):
    ...
    gt_vis_bboxes[:, [0, 2]] = np.clip(gt_vis_bboxes[:, [0, 2]], 0, img_width-1)
    gt_vis_bboxes[:, [1, 3]] = np.clip(gt_vis_bboxes[:, [1, 3]], 0, img_height-1)
    ...
    if  np.random.randint(0, 2) == 0:
        ...
        if len(gt_bboxes) > 0:
            ...
            gt_vis_bboxes[:, [0, 2]] = img_width - gt_vis_bboxes[:, [2, 0]]
        if len(gt_bboxes_ignore) > 0:
            gt_bboxes_ignore[:, [0, 2]] = img_width - gt_bboxes_ignore[:, [2, 0]]

    img, gt_bboxes, gt_vis_bboxes, gt_bboxes_ignore = resize_image(img, gt_bboxes, gt_vis_bboxes, gt_bboxes_ignore, scale=(0.4, 1.5))

    if img.shape[0] >= size_train[0]:
        img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore = random_crop(img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore, size_train, limit=16, small_box_to_ignore=small_box_to_ignore)
    else:
        img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore = random_pave(img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore, size_train, limit=16, small_box_to_ignore=small_box_to_ignore)
    ...
    return img, gt_bboxes, gt_vis_bboxes, gt_labels, gt_bboxes_ignore
